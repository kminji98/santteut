...
if(isset($_GET["mode"])&&$_GET["mode"]=="search"){
  ...
  $sql = "SELECT * FROM `mt_information` WHERE `$find_option` like '%$q_find_input%';";

}else{
  $sql = "SELECT * FROM `mt_information` order by num desc";
}
  ...
  <body>
    ...
    <div class="page_button_group">
      ...
      if( $start_page > PAGE_SCALE ){
        ...
        echo( '<a href="mt_information_list.php?page=1"><button type="button" name="button" title="처음으로"><<</button></a>' );

        // 이전 블럭 클릭 시 -> 현재 블럭의 시작 페이지 - 페이지 스케일
        // 현재 6 page 인 경우 '<(이전블럭)' 클릭 -> $pre_page = 6-PAGE_SCALE  -> 1 페이지로 이동
        $pre_block= $start_page - PAGE_SCALE;
        if(isset($_GET['mode']) && $_GET['mode']=="search"){
          echo( '<a href="mt_information_list.php?mode=search&find_input=$find_input&page='.$pre_block.'"><button type="button" name="button" title="이전"><</button></a>' );
        }else{
          echo( '<a href="mt_information_list.php?page='.$pre_block.'"><button type="button" name="button" title="이전"><</button></a>' );
        }
      }
      ...
      for( $j = $start_page; $j <= $end_page; $j++ ){
          ...
          if ( $j == $page ){
            ...
            echo( '<a href="mt_information_list.php?mode=search&find_input=$find_input&page='.$j.'"><button type="button" name="button">'.$j.'</button></a>' );
          }else{
            echo( '<a href="mt_information_list.php?page='.$j.'"><button type="button" name="button">'.$j.'</button></a>' );
          }
      }
      ...
      if( $total_pages > $end_page ){
        ...
        if(isset($_GET['mode']) && $_GET['mode']=="search"){
          echo( '<a href="mt_information_list.php?mode=search&find_input=$find_input&page='.$next_block.'"><button type="button" name="button">></button></a>' );
        }else{
          echo( '<a href="mt_information_list.php?page='.$next_block.'"><button type="button" name="button" title="다음">></button></a>' );
        }

        //맨끝페이지로 이동
        echo( '<a href="mt_information_list.php?page='.$total_pages.'"><button type="button" name="button" title="맨끝으로">>></button></a>' );
      }
      ?>
    </div>
